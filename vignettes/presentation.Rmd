...
A minor thing but one that caused significant headaches is the wonderful `cut2()` function from [Hmisc](http://cran.r-project.org/web/packages/Hmisc/index.html). `cut2()` is a more flexible boundary generating function that produces (IMO) better intervals i.e.
...
To avoid this I wrote a `sanitise()` function to replace the `sanitize()` function from [knitr](http://cran.r-project.org/web/packages/knitr/index.html) so that it will escape any `[` characters a table output from R. Whilst I was giving it a UK name (to save function masking issues), I also extended it to correctly handle the GBP symbol (```{r} \u00a310```) since that was also an issue for us.
...
optiRum contains a `theme_optimum()` (after the company, not an adjective) that changes the ggplot2 default chart look. The ggplot2 themes are good to begin with, and others have extended it further e.g. [ggthemes](http://cran.r-project.org/web/packages/ggthemes/index.html), but we wanted a standard within our company that we could use for charts.  There were some considerations:
